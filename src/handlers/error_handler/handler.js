...
var help_note = 'Hi! This bot will allow you to manage and run tournaments over discord and challonge with minimal admnistrative work.\n\n' +
				'I have been coded with natural language processing, so I am pretty decent at figuring out your intentions.\n\n' +
				'\n\n However, here is a list of commands you can use if I am misunderstanding you: \n+HELP: brings up this page' +
				'\n+CREATE_TOURNEY: The first step to creating a new tournament\n' +
				'+START_TOURNEY: begin the tournament once the parameeters are set and all of the participants have entered.\n' +
				'+JOIN_TOURNEY <TEAM NAME/NICK NAME>: Join the tourney with the given team name / join the given team.\n'+
				'+DROP_TOURNEY: Drop out of an active tournament\n' +
				'+MATCH_REPORT_WIN: Report a victory!\n' +
				'+MATCH_REPORT_LOSE: Report a defeat.\n' +
				'+REPORT <@username>: Report the given user\n' +
				'+RESOLVE: Force the resolution of a dispute (tourney creator only)\n';
...
var drop_tourney_err = 'you are not currently a participant in this tournament, and thus cannot drop out of it.\n';
...
var canada_err = 'Lazer Bears are extremely banned from this tournament. No cheating!!!'
...
function intended(team_id, initiator_id, msg, t_status){
	if(msg.parsed_msg.parse === parser_constants['HELP']){
		err_msg = help_note;
		msg.reply(err_msg);
		return false;
	}
	...
	/*if(msg.parsed_msg.parse === parser_constants['DROP_TOURNEY'] && parseInt(team_id, 16) > 10){
		err_msg = drop_tourney_err;
		msg.reply(err_msg);
		return false;
	}*/
	...
	if(msg.parsed_msg.parse === parser_constants['O_CANADA']){
		err_msg = canada_err;
		msg.reply(err_msg);
		return false;
	}
